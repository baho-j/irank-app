...
/**
 * Serialises with object keys in a fixed order, so two devices that built the
 * same record digest it identically regardless of insertion order.
 */
...
/**
 * A stable, order-independent digest covering each entity's contents.
 *
 * The payload is included deliberately: a digest over identifiers alone would
 * accept a bundle whose ballot scores were altered in transit, since the ids
 * and timestamps would still line up.
 */
export function checksumOf(entities: BundleEntity[]): string {
  const canonical = entities
    .map(
      (entity) =>
        `${entity.type}:${entity.id}:${entity.updated_at}:${stableStringify(entity.payload)}`
    )
    .sort()
    .join("|");
